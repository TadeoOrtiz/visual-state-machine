class_name State extends RefCounted
## Abtract class for create states
##
## This class contains the methods and properties 
## necessary to create behavioral states for other nodes.
##

var _name: String

## Target of state
var target: Node

## Inputs list linked to custom function
var inputs: Array[StateInput]

## Outputs list of the state
var outputs: Array[StateOutput]

## Use init as a ready
func _init() -> void:
	pass

## Called when [StateOutput] is connected
func _on_enter() -> void:
	pass

## Called when any [StateOutput] in state is emited
func _on_exit() -> void:
	pass

## Same function of [method Node._process]
func _process(delta: float) -> void:
	pass

## Same function of [method Node._physics_process]
func _physics_process(delta: float) -> void:
	pass

## Same function of [method Node._input]
func _input(event: InputEvent) -> void:
	pass

## Set state name
func set_name(n: String) -> void:
	_name = n

## Return state name
func get_name() -> String:
	return _name

func _to_string() -> String:
	return get_name()

func _notification(what: int) -> void:
	match what:
		StateMachine.NOTIFICATION_STATE_EXIT:
			_on_exit()
